...
# Bind tools only when enabled
if NO_TOOLS or not TOOLS:
    llm_with_tools = llm.with_config({"run_name": "chat_model"})
else:
...
async def predict_meci_hf_async_with_resampling(
    ...
    per_doc: Dict[int, Dict[str, List[str]]] = {}  # {doc_idx: (tuple string id){"y_true": [...], "y_pred": [...]}}
    ...
    async def run_batch_with_resampling(
        doc_idx: tuple[str, int], batch_idx: int, doc_text: str,
        ...
        if not enable_resampling:
            ...
            pred_map = {(Ti, Tj): lab for (Ti, _gold, Tj), lab in zip(batch, y_pred)}
            per_pair_rows: List[Dict[str, Any]] = []
            for (Ti, gold, Tj) in batch:
                per_pair_rows.append({
                    ...
                    "Tj": Tj,
                    "gold": gold,
                    "pred": pred_map.get((Ti, Tj), "NoRel"),
                })
            return _doc_idx, _lang, y_true, y_pred, per_pair_rows
        ...
        # NEW: build flat per-pair rows for logging
        per_pair_rows: List[Dict[str, Any]] = []
        pps = agg_stats.get("per_pair_stats", {}) or {}
        for (Ti, gold, Tj) in batch:
            key = f"{Ti},{Tj}"
            stat = pps.get(key, {}) if isinstance(pps, dict) else {}
            per_pair_rows.append({
                "doc_idx": doc_idx[1],
                "id": doc_idx[0],
                "lang": lang,
                ...
                "gold": gold,
                "pred": pred_map.get((Ti, Tj), "NoRel"),
                "vote_counts": stat.get("vote_counts"),
            })

        return doc_idx, lang, y_true, y_pred, per_pair_rows


    async def run_single_batch_independent(
        doc_idx: tuple[str, int], batch_idx: int, run_idx: int, doc_text: str,
        spans: Dict[str, str], batch: List[Tuple[str, str, str]], lang: str
    ):
        ...
        async def run_attempt():
            ...
            async with sem:
                ...
                thread_id = f"meci::resample::{split}::{doc_idx[1]}::b{batch_idx}::r{run_idx}::{uuid.uuid4().hex[:8]}"
                ...
                with tracing_context(
                    ...
                    arr = _coerce_arr(arr)
                    # Strict shape check; force retry if malformed
                    if not isinstance(arr, list) or not all(isinstance(x, dict) and "pair" in x and "label" in x for x in arr):
                        print("Wong Json generation or decoding")
                        raise ValueError("prediction format not list[dict{pair,label}]")
    ...
    with tracing_context(
        ...
        for idx, row in enumerate(it):
            ...
            # Use HF dataset "id" column as the document identifier (string)
            if "id" not in row:
                raise KeyError('Dataset row is missing required "id" field.')
            doc_id = (str(row["id"]), idx)
            ...
            print(f"Processing document {doc_id[1]} ({lang}) with {len(gold_triples)} pairs - Resampling: {n_resampling_runs if enable_resampling else 1} runs", end="\r")
            
            for b_idx, batch in enumerate(chunked(gold_triples, pair_batch_size)):
                tasks.append(asyncio.create_task(
                    run_batch_with_resampling(doc_id, b_idx, doc_text, spans, batch, lang)
                ))
        ...
        for coro in asyncio.as_completed(tasks):
            doc_idx, lang, y_true, y_pred, per_pair_rows = await coro
            completed_batches += 1
            ...
            all_pair_predictions.extend(per_pair_rows)
    ...
    report = {
        ...
        "per_pair_predictions": all_pair_predictions,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--max_examples", type=int, default=CONFIG.get("inference", {}).get("max_examples", 0))
    ...
    res.pop("resampling")
    res.pop("per_pair_predictions", None)
